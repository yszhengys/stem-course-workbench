...
class LabPublicationError(InvalidInputError):
    """Raised when a complete current Lab proposal lacks exact human approval."""
...
class PublicationService:
    ...
    async def assert_labs_ready(self, scope: DraftScope) -> None:
        """Require every current persisted Lab proposal to be exactly approved."""
        ...
        if any(
            isinstance(row, dict) and row.get("proposal_hash") is None
            for row in rows
        ):
            raise LabPublicationError("The current Lab proposal hash is invalid.")
        try:
            labs = tuple(Lab(**row) for row in rows if isinstance(row, dict))
        except (TypeError, ValidationError, ValueError) as exc:
            raise LabPublicationError("The current Lab approval data is invalid.") from exc
        if not labs:
            raise LabPublicationError(
                "At least one complete approved Lab proposal is required."
            )
        keys: set[str] = set()
        for lab in labs:
            if (
                ...
                raise LabPublicationError("The Lab set contains a stale chapter Lab.")
            try:
                payload = canonical_lab_proposal_payload(lab.payload)
                expected_hash = canonical_lab_proposal_hash(payload)
            except (TypeError, ValidationError, ValueError) as exc:
                raise LabPublicationError(
                    "The current Lab proposal is invalid."
                ) from exc
            key = payload.get("key")
            if not isinstance(key, str) or key in keys:
                raise LabPublicationError("The current Lab stable keys are invalid.")
            keys.add(key)
            if payload.get("pedagogy") is None:
                raise LabPublicationError("Complete Lab pedagogy is required.")
            if lab.proposal_hash is None or lab.proposal_hash != expected_hash:
                raise LabPublicationError("The current Lab proposal hash is invalid.")
            if (
                lab.approved_hash != lab.proposal_hash
                or lab.approved_at is None
                or lab.approval_reason is None
            ):
                raise LabPublicationError(
                    "The current Lab proposal requires exact human approval."
                )
