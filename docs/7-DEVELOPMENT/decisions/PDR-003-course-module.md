...
8. **Evidence stays out of Git.** Raw course material lives only in gitignored local directories (`course_evidence/`, `course_originals/`), regardless of repository visibility. The source repository may be public; only source code, documentation, and explicitly synthetic or redistributable fixtures may be tracked.
