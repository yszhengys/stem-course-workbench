# STEM Course Workbench V2 中文使用说明

STEM Course Workbench 是基于 Open Notebook 的本地优先教材与学习工作台。它把 PDF/PPTX 教学材料转换为可追溯的数学或物理课程：先在 Build 模式建立 Docling 证据、生成并人工批准大纲、编辑和发布课程，再在 Learn 模式阅读、练习、实验、复习和使用带引用的章节导师。

当前 Workbench 产品版本为 `2.0.0-dev`（development），兼容的上游基线为 Open Notebook 1.14.0。两个版本分别维护：Workbench 版本描述本项目功能，上游基线描述继承的 Python 包与架构，不能互相替代。

V2 不会自动替代教师。生成内容必须有来源或明确标为教学推导；客观题由确定性规则评分，解释题和证明题只提供建议；无法可靠验证的内容会阻塞发布或要求人工检查。

## 最短启动路径

适用环境：Apple Silicon Mac（ARM64）和已启动的 Docker Desktop。

在项目目录运行：

```bash
./scripts/course-workbench.sh
```

如果保存的界面语言或浏览器 locale 不是中文，可在侧边栏的语言菜单切换为中文；这不会改变其他 Open Notebook 用户的默认语言。

脚本会：

1. 安全创建或补全私有 `.env`，生成非占位加密密钥，并把权限设为 `600`；
2. 按锁文件准备 Python 和前端依赖；
3. 只在 Docker 中启动 SurrealDB；
4. 在宿主机后台启动 API、worker 和前端；
5. 等所有服务及课程入口真正就绪后，打开 `http://127.0.0.1:3000/courses/new`。

它不会自动创建课程、上传材料、调用模型、删除数据，也不会提交 `.env` 或本地材料。

### 前置条件

- macOS Apple Silicon（ARM64），建议预留至少 25 GB 可用空间。
- Docker Desktop 已安装并已启动；Docker 只用于 SurrealDB。
- Python 环境由锁文件管理。启动器优先使用 `./.tools/bin/uv`，其次使用 `PATH` 中有效的 `uv`；两者都不存在时，会下载固定的官方 macOS ARM64 资产、验证内置 SHA256 后安装到被 Git 忽略的 `.tools/bin/`，不会执行远程 shell。
- Node.js 20 或更新版本，以及 npm。
- 若使用 Codex：本机 Codex CLI 已安装并完成登录。
- 若使用 Ollama：Ollama 已启动，并提前拉取 `qwen3.5:9b` 或 `gpt-oss:20b`。

首次运行可能需要下载已校验的 `uv`、安装锁定依赖和下载 Docling/Ollama 本地模型，因此会比后续启动慢。启动器不会安装 Homebrew，也不会删除已有模型；`uv` 下载或校验失败时会停止并显示错误，不会运行未验证文件。

常用命令：

```bash
./scripts/course-workbench.sh start --no-open
./scripts/course-workbench.sh status
./scripts/course-workbench.sh logs api
./scripts/course-workbench.sh logs worker
./scripts/course-workbench.sh logs frontend
./scripts/course-workbench.sh restart
./scripts/course-workbench.sh stop
```

`stop` 只停止本项目启动并验证归属的进程和 SurrealDB 容器，不删除数据库、证据或课程内容。

## 首次创建课程

1. 启动后，在“新建课程”页面输入课程名称。
2. 学科选择“数学”或“物理”。课程内容语言可以选择简体中文或英文；界面语言与课程内容语言相互独立。
3. 点击“创建课程”。系统会同时创建并绑定一个 Open Notebook Notebook，然后进入课程大纲工作区。
4. 如果材料尚未存在，点击页面 CTA 会打开这个 Course 自己的 Notebook；在那里上传 PDF 或 PPTX 会自动保留 Notebook 上下文，再回到课程大纲页。
5. 在来源选择器中选择文件；必要时也可手动输入已经属于该 Course Notebook 的 `source:...` ID。系统会拒绝其他 Notebook、非 PDF/PPTX 或没有本地原文件的 Source。选择 `PRIMARY`（主要教材）或 `SUPPLEMENT`（补充材料）。
6. 点击“构建证据”，等待任务状态从 queued/running 变为 succeeded。失败时页面显示后台错误，不会自动换模型或静默重试。

课程只接受带原始文件的 `.pdf` 和 `.pptx` Source：

- 旧 `.ppt` 不支持，请先转换为 `.pptx`。
- URL、纯文本和没有原始文件的 Source 不会出现在课程选择器中。
- 损坏、加密、空文件或 OCR 无有效证据时会明确失败，不会继续生成无依据内容。

## 证据锚点

Docling 在本机解析材料并建立证据锚点：

- PDF 使用从 1 开始的页码；PPTX 使用从 1 开始的幻灯片号。
- 锚点保存短引文、块标识、归一化边界框、源文件 SHA256 和引文哈希。
- 相同源文件和相同内容会得到确定性的锚点 ID。
- PDF 锚点的“打开来源”会通过 Course 归属与当前哈希检查后，在原 PDF 的对应页打开；PPTX 保留原始文件下载按钮，并优先显示本机渲染的幻灯片图像。
- PPTX 图像预览需要本机安装 LibreOffice（macOS 默认位置 `/Applications/LibreOffice.app`）；项目内的 PDFium 会把其转换结果栅格化为受限 PNG。缺少 LibreOffice、转换超时或运行时不可用时，证据仍可使用，但页面会显示“纯文本预览”徽标和由 Docling 引文生成的安全 SVG。
- 幻灯片图像上的橙色框来自 Docling 的归一化 bbox，只用于帮助找到锚点位置；它不裁剪图片、不接受点击，也不代表 OCR 或分类一定正确。没有有效 bbox 时不会显示框。
- 图像和纯文本预览都不执行脚本、HTML、外部资源、宏、嵌入对象或模型生成代码；页面不会显示本机绝对路径。包含宏、ActiveX、OLE、外部关系或危险 ZIP 结构的 PPTX 会被拒绝，而不是交给渲染器。
- 原文件保持不变；缓存位于 `notebook_data/course_evidence/`。

如果原文件内容变化，旧哈希不会被当作新材料继续使用，应重新构建证据。

### 补全书目信息并审计来源覆盖

关联材料后，大纲页会为每个 Source 显示书目编辑器。按原材料版权页或出版信息填写作者（每行一个或用分号分隔）、标题、版本、出版社、年份、DOI、ISBN 和许可；亲自核对后再勾选“已人工核对”。这个复选框只是你的审计声明，系统不会根据文件名或模型输出自动勾选，也不会替代页码/幻灯片证据锚点。

生成大纲后可查看“来源覆盖报告”。它只统计当前结构化大纲、章节、例题、练习和实验中明确保存的锚点，不把正文里看起来像引用的文字算进去。可组合筛选并下载确定性 JSON；报告哈希相同表示报告内容相同。常见标记：

| 标记 | 应如何处理 |
|---|---|
| 未使用 `unused` | 确认锚点是否应被引用，或是否可以从课程材料中排除。 |
| 低置信 `low_confidence` | 人工确认它究竟是定义、定理、例题、练习、答案、图表还是先修知识。 |
| 仅补充来源 `supplement_only` | 检查该用途是否也需要主要教材证据，或明确接受仅由补充材料支持。 |
| 缺少书目 `missing_bibliography` | 为该 Source 保存 Course 书目记录，并人工核对需要的字段。 |
| 章节无答案来源 `no_answer_source` | 检查该章是否确实没有教材答案；需要时补充或关联答案锚点。 |
| 超出生成审计边界 `generation_limit_exceeded` | 当前练习超过 500 项，缩小审计范围并增加人工抽查。 |

这些标记是证据覆盖提醒，不是内容质量分数、版权结论、公式正确性证明或学生掌握度。

## 生成并批准大纲

1. 勾选用于本课程的证据锚点。
2. 在模型选择器中选择当前显示为可用的模型。
3. 点击“生成大纲”，等待后台任务真正完成。
4. 检查章节树、学习目标、先修关系、概念依赖和每一章的引用。环依赖、无依据的核心概念或不合法的实验提案会阻止审批。
5. 确认无误后，必须原样输入：

```text
确认大纲
```

只有当前版本、正确状态和完全一致的确认短语能够通过。输入框末尾的一次换行（LF 或 CRLF）可以被容忍，内部换行、连续两个换行或单独的 CR 会被拒绝。批准后的版本不可变；后续重做会创建新版本并再次要求批准。

## 生成、审查和发布章节

打开已批准大纲中的一章：

1. 分别选择内容模型、独立审查模型和高风险定点复核模型，并确认所选证据锚点。默认依次为 Sol、Luna、Sol。
2. 生成章节。已就绪或已发布章节的“重新生成”会创建新版本，不覆盖旧内容。
3. 章节会包含学习用途、先修知识、目标、定义、公式、分步例题、误区、实验、练习、提示、答案后的迁移题、速查表和引用。
4. 请求独立审查。Luna 先输出结构化 findings；存在高风险或不确定项时，后台创建独立、可追踪的复核记录，只把必要条目和对应短引文定点交给所选 Sol 复核。Sol 失败会明确阻塞本章，不会自动换模型或降级。
5. 独立验证器检查公式、数值、单位/量纲、物理方向/参考系/边界以及引用哈希。无法解析的公式会成为阻塞性的 `manual_check`，不会被假定为正确。页面同时显示每个公式和展示答案的 L0–L3 验证来源；第二个模型的审查最多仍是 L1，不代表知识已经被证明正确。
6. 解决所有未确认的 `error`/`high` finding。warning 需要明确处理或确认原因。
7. 只有批准大纲中的当前章节、完整校验通过、题库满足发布条件、每个 Lab 完成精确人工审批且没有未解决阻塞项时才能发布。发布较早章节会正常完成但不会提前结束整门课程；发布最后一个当前章节时，系统自动再次核对全书引用、来源哈希、findings 和并发状态，并把课程晋级为可用状态。若最后一步曾中断，再次点击已发布的最后一章会安全修复晋级，不会重写章节。

### V2 题库与结构化编辑

V2 会把来源内容分类为定义、定理、例题、练习、答案、图表和先修知识，并为练习保存稳定键、来源题号、概念、答案类型、评分契约与六维难度。发布前请在章节 Build 页面检查：

- 来源基础题是否被保留，并有完整的证据锚点；
- 核心题和挑战题是否真的高于相应来源基线；
- 深迁移题是否改变表示、条件或推理结构，而不只是替换数字和名称；
- 数值、符号、单位、向量、集合和多部分题是否能被确定性复算；
- 证明题与解释题是否明确标为建议性反馈，不能自动授予掌握。

结构化编辑器只允许修改正文块、定义、公式、例题、练习、提示、迁移题和声明式 LabSpec。每次操作都带服务器修订令牌；其他窗口已经修改时会返回冲突，刷新后再编辑。保存后必须重新验证，局部改动会使相关公式、引用、题库或实验检查失效。批准或发布后的版本不能原地修改，需创建新版本。编辑器不是任意 HTML/WYSIWYG 编辑器，也不会执行粘贴的代码。

### 公式与显示答案的 L0–L3

Build 页面中的“学术核验审查”会为公式、例题答案和旧式练习答案显示诚实来源：

| 级别 | 表示什么 | 不表示什么 |
|---|---|---|
| L0 | 结构安全、格式可解析；手工改动后的初始状态 | 不表示数值或公式正确 |
| L1 | 生成物内部自洽，或第二个模型没有发现矛盾 | 不表示教材或现实世界正确 |
| L2 | 已由教材答案锚点或独立确定性求解器核对 | 不等于教师已人工签署 |
| L3 | 人工核对了页面当前显示的精确值和证据 | 只对记录的当前 artifact 快照有效 |

签署 L3 时必须复制页面显示的精确公式/答案，并填写核验理由与证据锚点。系统保存服务器 UTC 时间、当前 artifact SHA256 和理由；请求不能自行提交级别或时间。之后若修改该公式或答案，旧 L3 会自动失效并降为 L0，需要重新验证。独立模型审查始终不能自动升级到 L2/L3。

后台任务每 2 秒刷新一次，最长等待 30 分钟。超时只停止页面轮询；可用 `status` 和 `logs worker` 检查 worker，再手动刷新。系统不会自动换供应商或自动降级。

## 模型选择

页面只允许选择实际可用的选项：

- `codex_cli`：本机 Codex CLI。默认内容模型为 `gpt-5.6-sol`，审查模型为 `gpt-5.6-luna`；必须已登录。
- `ollama`：本机 Ollama 中实际安装且在线的 `qwen3.5:9b` 或 `gpt-oss:20b`。
- `open_notebook`：已经在 Open Notebook 中注册的模型，包括用户主动配置的 DeepSeek 等供应商。这里选择的是实际注册后返回的 `model:...` 记录，不把某个 DeepSeek 型号写死为默认值。

`.env` 中 `OPEN_NOTEBOOK_COURSE_ALLOW_REAL_MODELS=1` 只允许你点击生成按钮后调用所选模型。启动、浏览课程或构建普通页面不会调用模型。请求一旦提交，后台不会悄悄换模型；登录、配额、离线或输出格式问题都会明确失败。

如需在首次安装或模型更新后主动复核四个模型，可运行下面的显式冒烟测试。它会实际调用两个 Ollama 模型和 Codex Sol/Luna，各自只生成一个极小的结构化结果，因此应在确认本地内存与 Codex 配额可用后执行：

```bash
OPEN_NOTEBOOK_RUN_REAL_MODEL_SMOKE=1 ./.tools/bin/uv run --env-file .env \
  pytest tests/course/test_real_model_adapters_smoke.py -v
```

普通测试默认跳过这四个真实调用，不会意外消耗模型资源。

## 安全交互实验

V1 只支持五种声明式实验：

- 函数图像 `function_plot`
- 参数曲线 `parametric_curve`
- 向量场/向量图 `vector_field`
- 几何 `geometry`
- 运动学 `kinematics`

前端用受限 JSON、数学表达式 AST 和自有白名单解释器绘制 Canvas/SVG。系统不执行模型生成的 JavaScript、HTML、`eval` 或 `Function`，并限制控件、表达式、对象、数值范围和总采样点。

每个新实验还必须完整列出学习目标、先修概念、变量/单位/范围、预测问题、操作步骤、预期观察、学生提交内容、评分规则、适用/误差边界和无障碍文字/数据表替代。Build 页会显示整份教学方案及其 SHA256。教师检查后填写理由，并原样输入：

```text
确认实验方案
```

批准记录由服务器绑定到当前 `proposal_hash`，保存 UTC 时间和理由。更改标题、表达式、控件、对象或任何教学字段都会生成新哈希并原子清空旧审批，页面显示“尚未批准或批准已失效”，必须重新检查和批准。旧版已发布 Lab 仍可阅读；若要修改或重新批准，需通过不可变学习升级/新版本流程，不能原地改写。

## Learn 模式：阅读、练习和复习

课程发布后，回到“课程”列表点击“学习模式”。学习总览只显示当前已发布版本，并提供继续学习、章节导航、概念掌握度和到期复习队列。章节学习页依次提供教材正文、来源预览、核心练习、安全实验、章节导师和笔记。

阅读位置会自动保存，但阅读位置、章节完成度和概念掌握度是三类不同记录。只阅读正文或手动标记章节完成，都不会授予“已练习”或“已掌握”。掌握状态固定为：

```text
未开始 → 学习中 → 已练习 → 已掌握 → 待复习
```

掌握规则由服务器从不可变学习事件重放计算：

- 只有核心题答对才进入练习进度；
- 查看全部分层提示后答对，最多到“已练习”；
- 同一概念至少答对两道不同教材级题，且至少一道未揭示答案，才能“已掌握”；
- 揭示核心题答案后，必须独立完成对应深迁移题，才能继续获得该概念的掌握进度；
- 复习间隔依次为 1、3、7、14、30 天；答错会回退，揭示答案不会推进间隔；
- 重复提交使用幂等键，不会重复增加掌握进度；旧课程版本的学习请求会被拒绝并提示刷新。

数值、符号、单位、向量、集合和多部分答案由确定性 grader 判定。证明和解释答案会保存并可获得建议，但不会由模型直接判定为掌握。

## 带引用的章节导师

Learn 章节页的导师只读取当前已发布章节、当前版本和服务器选定的证据锚点，可用于解释概念、诊断一次练习尝试或给出下一层提示。使用时请先选择页面显示为可用的模型，再创建会话。

- 每个事实性陈述都必须带来源锚点；证据不足时导师会明确拒答，不会用模型常识补写。
- “解释”模式中模型只能选择当前章节的证据锚点，页面实际展示的是服务器保存且校验过的原始证据摘录，不直接展示模型自由改写的文字。
- “下一层提示”只返回该题已经发布的分层提示，并绑定页面上当前练习作答；“诊断”只读取同一道题、同一概念、同一次作答的真实确定性评分记录。导师提示会写入学习事件，因此会计入提示使用限制。
- 默认不展示完整答案。你明确要求揭示时，系统会记录事件，并激活相应的深迁移要求。
- 导师不能跨课程、跨章节或读取旧版本证据；课程版本变化后旧会话转为只读。
- 材料文本按不可信输入处理，其中的“忽略规则”等指令不会获得控制权。
- 模型失败会明确显示，不自动换模型或供应商。
- 完全相同的失败请求会使用原幂等身份安全重试；请求身份会先以不可变记录保存，并用执行租约避免两个并发重试重复调用模型。若相同请求仍在运行，页面会收到“正在处理，请稍后重试”，而不会悄悄再调用一次。即使学习事件已写入但页面尚未收到导师回复，也不能把同一身份换成另一组参数重用。如果你修改意图、文字、练习、概念或作答，页面会生成新身份，后端也会拒绝同一身份对应不同请求。

## 笔记、进度和练习记录

- 笔记绑定课程、章节稳定键和内容块稳定键。
- 章节重新生成后，仍存在的稳定键会继续关联；消失的块会把笔记标为 `orphaned`，可在界面重新挂接到当前有效块。
- 进度保存章节/块位置、完成状态和更新时间。
- 练习记录保存回答、已查看提示、是否揭示答案、评分结果和迁移题状态；掌握快照可由原始学习事件重新计算。
- 历史尝试不会因章节重新生成而被覆盖。

## 手动导出与导入课程

课程列表底部提供 `.stemcourse` 便携包：

1. 选择课程并创建导出；导出完成后下载 `.stemcourse` 文件。
2. 可选择是否包含原始 PDF/PPTX。只有明确选择“包含原始材料”时，源文件才进入包；默认只迁移课程结构、证据记录、Course 书目和不可逆的视觉缓存身份摘要。
3. 在另一份本项目中选择文件并导入。导入会先检查格式、路径、大小、文件数量、manifest、全部 SHA256 和记录数，再在一个数据库事务中创建新的 Course、Notebook、Source 和关联记录 ID。
4. 导入不会覆盖同名或同 ID 的现有课程；成功后刷新课程列表，从 Build 或 Learn 模式进入。为避免复用其他机器的缓存，所有 SVG/PNG 路径都会被清空，PPTX 暂时显示为纯文本状态；重新构建证据后才恢复视觉预览。

包内不会包含 `.env`、凭据、日志、模型、模型缓存、运行命令、本机绝对路径、预览 PNG/SVG 或中间 PDF。视觉 manifest 只包含来源哈希、幻灯片号、可视状态和确定性身份哈希。不要手动修改扩展名或解压后重打包；夹带路径、未知字段或完整性校验失败的文件都会在写入数据库前被拒绝。V2 只提供手动导入/导出，不是自动备份系统。

早期 V2 导出的包可能没有 Course 书目与视觉 manifest；系统会将它们安全地解释为“无书目、纯文本预览”并继续导入。导入后补全书目并重新构建证据即可。

## 服务与健康检查

| 服务 | 地址/检查 | 运行位置 |
|---|---|---|
| SurrealDB | `http://127.0.0.1:8000/health` | Docker |
| API | `http://127.0.0.1:5055/health` | macOS 宿主机 |
| API 文档 | `http://127.0.0.1:5055/docs` | macOS 宿主机 |
| 前端 | `http://127.0.0.1:3000` | macOS 宿主机 |
| 新建课程 | `http://127.0.0.1:3000/courses/new` | macOS 宿主机 |
| worker | `./scripts/course-workbench.sh status` | macOS 宿主机 |

运行日志位于 `.runtime/course-workbench/logs/`。PID 和进程组元数据也在 `.runtime/course-workbench/`，由启动脚本管理；不要手工复用这些文件控制其他进程。

## 手动开发启动（可选）

通常不需要打开四个终端。如果要调试，可分别运行：

```bash
make database
make api
make worker-start
make frontend
```

API 和 worker 使用 `.env`，worker 默认并发为 5。Course 的 Docling 与重型本地模型任务另有领域级互斥锁，因此不需要把所有 Open Notebook 后台任务全局串行化。

### 维护者的干净克隆预检

发布前应在一次性的全新 clone 中运行：

```bash
./scripts/verify-clean-clone.sh "$PWD"
```

该脚本只验证经过 SHA256 校验的固定 `uv` 引导、锁定的 Python 运行时依赖和前端依赖安装，不读取 `.env`、不启动 Docker/API/worker/前端，也不调用 Codex、Ollama 或其他模型。为避免把本机缓存误当作安装成功，它会拒绝已经含有 `.tools/` 或 `.venv/` 的目录；日常使用仍运行 `./scripts/course-workbench.sh`。相同预检由 GitHub Actions 的 `macos-14` Apple Silicon runner 执行。

### 维护者的发布质量门

仓库只提交两份为本项目原创并以 CC0-1.0 释出的教学二进制金样本：`tests/course/fixtures/gold/stem-evidence-gold.pdf`（2 页）和 `stem-evidence-gold.pptx`（3 页）。固定 SHA256、预期公式、答案、图/低文本页和 bbox 要求记录在同目录的 `manifest.json`；它们用于测试当前解析链，不代表任意教材都能正确识别。

发布候选至少运行：

```bash
./.tools/bin/uv run pytest tests/ -q \
  --cov=open_notebook --cov=api --cov-fail-under=75
./.tools/bin/uv run ruff check .
./.tools/bin/uv run python -m mypy .
./.tools/bin/uv run python scripts/check_md_links.py
./scripts/verify-course-migration-gate.sh

OPEN_NOTEBOOK_RUN_REAL_DOCLING_SMOKE=1 ./.tools/bin/uv run \
  pytest tests/course/test_real_docling_preview_smoke.py -v
OPEN_NOTEBOOK_RUN_REAL_PPTX_VISUAL_SMOKE=1 ./.tools/bin/uv run \
  pytest tests/course/test_pptx_visual_renderer.py -k real -v

cd frontend
npm run test:coverage
npm run test:e2e
npm run lint
npm run build
```

首次在本机运行浏览器门且 Chromium 尚未安装时，先在 `frontend/` 执行 `npm run test:e2e:install`。迁移脚本只创建随机端口的临时 SurrealDB/RocksDB，验证升级、重启、降级、再升级和失败回滚；它不会连接或删除现有 `surreal_data/`。

这些命令不证明学习效果、教材级知识正确性、用户材料许可或完整 WCAG 2.2 AA。自动键盘/axe 门通过后，正式无障碍声明仍需完成并记录日期的[人工清单](../7-DEVELOPMENT/course-accessibility-checklist.md)。F01–F10 的当前证据与剩余产品决策见[审查整改状态](../7-DEVELOPMENT/review-remediation-status.md)。

## 数据、隐私与备份

本地重要目录：

- `surreal_data/`：SurrealDB 数据。
- `notebook_data/`：原始/派生内容和 Course 证据缓存。
- `.env`：本机凭据和加密密钥。
- `.runtime/`：可删除的运行日志和进程元数据，不是课程数据。

这些目录、原始教学材料、模型缓存和凭据都被 Git 忽略。不要把它们强制加入提交，也不要把密钥粘贴到 issue、PR 或日志中。

GitHub 仓库公开只会公开已提交的源代码、文档和明确可再分发的合成测试材料，不会自动上传上述本地目录。Course Workbench V2 是单用户、本地优先的开发配置；在完成独立身份认证、访问控制、CORS、TLS、数据库凭据和网络边界加固之前，不要把 SurrealDB、API 或前端直接暴露到公网。

下载的来源覆盖 JSON 会包含 Source/anchor ID、来源内容 SHA256、页/幻灯片号、块键、bbox、分类、用途和标记，但不包含短引文、原始文件或本机路径。`.stemcourse` 若选择包含原始材料，就会包含 PDF/PPTX 原文；分享前必须确认版权、许可和接收方。书目“已人工核对”仅记录你的检查状态，不构成授权或版权证明。

备份前先执行 `./scripts/course-workbench.sh stop`，再用你自己的加密存储工具同时备份 `surreal_data/`、`notebook_data/` 和 `.env`。恢复时三者应来自同一个时间点。同步上游代码前先提交自己的代码改动，然后从 `upstream` 获取并在功能分支集成；不要用会覆盖本地数据的 Git 命令处理上述目录。

## 常见故障

| 现象 | 处理 |
|---|---|
| 页面空白或一直检查连接 | 运行 `status`；查看 `logs api` 与 `logs frontend`；确认 `5055/health` 和 `3000/courses/new` 可访问后刷新。 |
| Docker 未运行 | 启动 Docker Desktop，再重新运行启动脚本。 |
| 8000/5055/3000 被占用 | 脚本会拒绝接管不属于本项目的服务。先确认并正常停止占用者，不要按端口强杀。 |
| 没有可选材料 | 到 Sources 上传 PDF/PPTX，并把它加入与课程同名的 Notebook；手动 ID 也必须已经属于该 Notebook。 |
| `.ppt` 被拒绝 | 用 PowerPoint/LibreOffice 转换为 `.pptx`。 |
| Docling 报错或无锚点 | 检查文件是否损坏、加密或为空；查看 worker 日志；确认本地 Docling 依赖和模型缓存可用。 |
| PPTX 只显示“纯文本预览” | 证据仍可用；如需幻灯片图像，请安装 LibreOffice 到 `/Applications/LibreOffice.app`，然后重新构建该 Source 的证据。 |
| PPTX 预览缺失或无法加载 | 重新构建该 Source 的证据；系统会拒绝陈旧哈希、缺失文件、异常缓存路径、符号链接和身份摘要不一致，不会改用不受控路径。 |
| bbox 高亮位置不准确 | bbox 来自 Docling，只是定位提示。对照原 PPTX 人工核对；不要把高亮框当作语义或公式正确性证明。 |
| 来源覆盖出现标记 | 在大纲页按标记筛选，核对主要/补充来源、书目和答案锚点；报告不是自动评分，不能用“无标记”替代人工审查。 |
| 模型不可选 | Codex 需安装并登录；Ollama 需运行且已拉取受支持模型；Open Notebook 模型需先注册。 |
| 任务一直 queued | 检查 worker 状态和日志；不要重复点击创建多个任务。相同活动输入会复用原任务。 |
| 任务 failed | 页面会显示明确错误；修复登录、配额、离线、无效 JSON 或证据问题后，再显式重试。系统不自动降级。 |
| 无法批准大纲 | 确认当前状态为 `outline_ready`，版本未过期，并精确输入“确认大纲”。 |
| 无法发布章节 | 解决所有未关闭的高风险/错误 finding、公式/单位/引用阻塞项，并确保使用当前批准大纲中的最新章节。 |
| 学习模式没有章节 | 只有当前完整发布的课程版本会出现在 Learn 模式；先回到制作模式完成验证和发布。 |
| 答对但未掌握 | 检查是否用了全部提示、揭示过答案、只完成了一道教材级题，或仍有未完成的深迁移要求。 |
| 导师拒答 | 当前章节证据不足、引用不合法、会话属于旧版本，或请求试图跨越课程边界；补充证据或创建当前版本会话。 |
| `.stemcourse` 导入失败 | 使用本项目直接导出的原文件；确认未修改 ZIP 内容、未夹带缓存路径，且文件大小/数量未超限制。失败导入不会留下半个课程。成功导入后仍需重建证据以恢复 PPTX 图像预览。 |

## V2 范围

V2 是单用户、local-first 的数学/物理教材与学习工作台。它不包含多用户/权限、多租户、协作编辑、云部署、自动备份、游戏化、视频课程、计算机科学、任意代码实验或用模型授予客观掌握。导师只在当前已发布章节内提供有引用的受控辅导；Open Notebook 的通用聊天仍是独立功能，不等同于课程导师。

上游 Open Notebook 的通用 Notebook、Source、Note、搜索、聊天和播客文档仍然适用；本说明只描述本分支新增的 Course 工作流。
