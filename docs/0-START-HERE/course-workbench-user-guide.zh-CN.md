...
只有当前版本、正确状态和完全一致的确认短语能够通过。批准后的版本不可变；后续重做会创建新版本并再次要求批准。

## 生成、审查和发布章节

打开已批准大纲中的一章：

1. 选择内容模型与审查模型，并确认所选证据锚点。
2. 生成章节。已就绪或已发布章节的“重新生成”会创建新版本，不覆盖旧内容。
3. 章节会包含学习用途、先修知识、目标、定义、公式、分步例题、误区、实验、练习、提示、答案后的迁移题、速查表和引用。
4. 请求独立审查。默认路线是 Sol 负责内容、Luna 负责结构化审查；高风险或不确定项只把必要条目和证据定点交给 Sol 复核。
...
7. 只有批准大纲中的当前章节、完整校验通过且没有未解决阻塞项时才能发布。
...
- `open_notebook`：已经在 Open Notebook 中注册的模型，包括用户主动配置的供应商；可选的 DeepSeek 配置使用 `deepseek-v4-pro`。
