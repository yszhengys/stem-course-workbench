...
`origin` 应始终指向私有 `yszhengys/stem-course-workbench`，`upstream` 指向 `lfnovo/open-notebook`。先获取上游并在功能分支集成；migration 24/25、增量 migration 26 及 Course facade 是兼容边界，不得改写已发布迁移。保留 MIT License 和上游历史。
