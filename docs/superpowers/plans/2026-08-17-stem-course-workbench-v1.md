...
- Keep the upstream Open Notebook UI and API; add one Course router, command modules, migration 24, and an isolated `open_notebook/course/` domain module.
...
- Run API, worker, and frontend on the host; run only SurrealDB in Docker. Serialize Docling and local Ollama jobs with worker concurrency 1.
...
2. Course schema, migration 24, and state-machine contracts.
...
6. Chapter pipeline, review, escalation, deterministic validation, and publish gate.
